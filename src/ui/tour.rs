...
fn slide_welcome(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(Line::from(styled(
        "   Learn programming offline, in your terminal. No browser, no account, no internet.",
        body_c,
    )));
    l.push(Line::from(styled(
        "   Real exercises, real compilers, real feedback — everything runs on your machine.",
        body_c,
    )));
    l.push(blank());
    ...
    l.push(dual(
        yellow,
        vec![muted("  Your code never leaves your", theme)],
        yellow,
        vec![muted("  Edit code right in the TUI.", theme)],
    ));
    l.push(dual(
        yellow,
        vec![muted("  machine. No telemetry.", theme)],
        yellow,
        vec![muted("  Line numbers, auto-scroll.", theme)],
    ));
    ...
    l.push(dual(
        green,
        vec![muted("  Colored diffs show where", theme)],
        green,
        vec![muted("  Ask questions while you", theme)],
    ));
    l.push(dual(
        green,
        vec![muted("  output differs. Errors shown.", theme)],
        green,
        vec![muted("  code. Enable in Settings.", theme)],
    ));
    l.push(dual_bot(green, green));
    l.push(dual_top("Real Sandboxing", cyan, "Progress Tracking", cyan));
    l.push(dual(
        cyan,
        vec![muted("  Exercises run in isolated", theme)],
        cyan,
        vec![muted("  Pick up where you left off.", theme)],
    ));
    l.push(dual(
        cyan,
        vec![muted("  sandboxes. Never affects system.", theme)],
        cyan,
        vec![muted("  Stats and celebrations.", theme)],
    ));
    ...
    l.push(dual(
        yellow,
        vec![muted("  Courses set up files, services,", theme)],
        yellow,
        vec![muted("  Free playground per lesson", theme)],
    ));
    l.push(dual(
        yellow,
        vec![muted("  ports — real dev environments.", theme)],
        yellow,
        vec![muted("  to experiment beyond exercises.", theme)],
    ));
    l.push(dual_bot(yellow, yellow));
...
fn slide_core_loop(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(Line::from(body(
        "   Every exercise follows the same cycle. No guessing what to do next:",
        theme,
    )));
    l.push(blank());
    ...
    // READ content
    {
        ...
        l.push(Line::from(vec![
            Span::raw("       ".to_string()),
            bold("│", cyan),
            styled(&format!("{:<w$}", label, w = bw), color),
            bold("│", cyan),
            body("   ────────→ ", theme),
            bold("│", green),
            styled(&format!("{:<w$}", rdesc, w = bw), rcolor),
            bold("│", green),
        ]));
    }
    l.push(Line::from(vec![
        ...
        bold("│", cyan),
        styled(
            &format!("{:<w$}", "  Read the lesson content.", w = bw),
            body_c,
        ),
        bold("│", cyan),
        Span::raw("             ".to_string()),
        bold("│", green),
        styled(
            &format!("{:<w$}", "  Write your solution in", w = bw),
            body_c,
        ),
        bold("│", green),
    ]));
    l.push(Line::from(vec![
        ...
        bold("│", cyan),
        styled(
            &format!("{:<w$}", "  [Space] reveals sections", w = bw),
            body_c,
        ),
        bold("│", cyan),
        Span::raw("             ".to_string()),
        bold("│", green),
        styled(
            &format!("{:<w$}", "  [e] opens the built-in", w = bw),
            body_c,
        ),
        bold("│", green),
    ]));
    l.push(Line::from(vec![
        ...
        bold("│", cyan),
        styled(
            &format!("{:<w$}", "  progressively as you go.", w = bw),
            body_c,
        ),
        bold("│", cyan),
        Span::raw("             ".to_string()),
        bold("│", green),
        styled(&format!("{:<w$}", "  inline editor.", w = bw), body_c),
        bold("│", green),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        ...
        Span::raw("             ".to_string()),
        bold(
            &format!("└{}┬{}┘", "─".repeat(bw / 2), "─".repeat(bw - bw / 2 - 1)),
            green,
        ),
    ]));
    ...
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        bold("│", yellow),
        styled(&format!("{:<w$}", "  4. SUBMIT", w = bw), green),
        bold("│", yellow),
        body("   ←────────  ", theme),
        bold("│", magenta),
        styled(&format!("{:<w$}", "  3. RUN", w = bw), blue),
        bold("│", magenta),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        bold("│", yellow),
        styled(
            &format!("{:<w$}", "  Grade when ready with [t].", w = bw),
            body_c,
        ),
        bold("│", yellow),
        Span::raw("             ".to_string()),
        bold("│", magenta),
        styled(
            &format!("{:<w$}", "  Test with [Enter] — see", w = bw),
            body_c,
        ),
        bold("│", magenta),
    ]));
    l.push(Line::from(vec![
        ...
        bold("│", yellow),
        styled(
            &format!("{:<w$}", "  ✓ Pass → next exercise.", w = bw),
            body_c,
        ),
        bold("│", yellow),
        Span::raw("             ".to_string()),
        bold("│", magenta),
        styled(
            &format!("{:<w$}", "  output without grading.", w = bw),
            body_c,
        ),
        bold("│", magenta),
    ]));
    l.push(Line::from(vec![
        ...
        bold("│", yellow),
        styled(
            &format!("{:<w$}", "  ✘ Fail → see feedback.", w = bw),
            body_c,
        ),
        bold("│", yellow),
        Span::raw("             ".to_string()),
        bold("│", magenta),
        styled(
            &format!("{:<w$}", "  Run as often as you want.", w = bw),
            body_c,
        ),
        bold("│", magenta),
    ]));
    ...
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[Enter]", cyan),
        body("  Run code (see output, no grading)        ", theme),
        styled("[r]", cyan),
        body("  Reset to starter code", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[t]    ", cyan),
        body("  Submit for grading (checks answer)       ", theme),
        styled("[?]", cyan),
    ...
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[Space]", cyan),
        body("  Reveal next lesson section               ", theme),
        styled("[Esc]  ", cyan),
        body("Return home", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[h]    ", cyan),
        body("  Progressive hints (reveals one at a time)", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[s]    ", cyan),
        body("  Skip exercise (come back later)", theme),
    ]));
    l.push(blank());
    l.push(Line::from(vec![
        body("   Only ", theme),
        styled("[t] submit", green),
        body(
            " counts toward progress — run freely, submit when confident.",
            theme,
        ),
    ]));
    l.push(Line::from(vec![
...
fn slide_exercise_types(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   Courses use six exercise types to build different skills:",
        theme,
    )));
    l.push(blank());
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 You get starter code with a TODO and build the solution.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 The code compiles/runs but produces wrong output.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 Key parts are replaced with blanks you fill in.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 Type what you think the code will print.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 Tests conceptual understanding without writing code.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(row(
        cyan,
        vec![body(
            "                 Write shell scripts regardless of the course language.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(dual(
        ...
        vec![
            styled("  → ", green),
            body("Opens the inline editor", theme),
        ],
        Color::Blue,
        vec![
            styled("  → ", Color::Blue),
            body("Opens a terminal prompt", theme),
        ],
    ));
    l.push(dual(
        green,
        vec![
            styled("  → ", green),
            body("Write/fix/fill-blank/predict", theme),
        ],
        Color::Blue,
        vec![
            styled("  → ", Color::Blue),
            body("Type shell commands live", theme),
        ],
    ));
    l.push(dual(
        green,
        vec![
            styled("  → ", green),
            body("[Enter] run, [t] submit", theme),
        ],
        Color::Blue,
        vec![
            styled("  → ", Color::Blue),
            body("[Enter] runs + validates", theme),
        ],
    ));
    l.push(dual(
        green,
        vec![styled("  → ", green), body("Choice: number keys", theme)],
        Color::Blue,
        vec![
            styled("  → ", Color::Blue),
    ...
    l.push(dual(
        green,
        vec![styled("  → ", green), body("Predict: type answer", theme)],
        Color::Blue,
        vec![
            styled("  → ", Color::Blue),
            body("[Ctrl+H] hints, [Esc] exit", theme),
        ],
    ));
    l.push(dual(green, vec![], Color::Blue, vec![]));
...
fn slide_editor(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   Press [e] to open the built-in editor — edit code right inside the TUI:",
        theme,
    )));
    l.push(blank());
    ...
    l.push(row(
        ...
        vec![
            styled("  5 │ ", muted_c),
            styled("    cout << \"Hello, World!\" << endl;", blue),
        ],
        FULL,
    ));
    l.push(row(
    ...
    l.push(dual(
        ...
        yellow,
        vec![styled("  →", green), body(" Esc saves and closes —", theme)],
    ));
    l.push(dual(
        cyan,
...
fn slide_feedback(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   When your code doesn't pass, LearnLocal shows you exactly why — not just \"wrong\":",
        theme,
    )));
    l.push(blank());
    ...
    l.push(dual(
        ...
        red,
        vec![
            styled("  error[E0308]:", red),
            styled(" mismatched", body_c),
        ],
    ));
    l.push(dual(
        cyan,
        vec![styled("  Got:      ", muted_c), styled("hello world", red)],
        red,
        vec![styled("  types", body_c)],
    ));
    l.push(dual(
        cyan,
        vec![styled("             ^     ^", yellow)],
        red,
        vec![styled("    --> ", body_c), styled("main.rs:3:12", muted_c)],
    ));
    l.push(dual(cyan, vec![], red, vec![]));
    l.push(dual(
        cyan,
        vec![body("  Color-coded line-by-line", theme)],
        red,
        vec![styled("    expected ", body_c), styled("`i32`", yellow)],
    ));
    l.push(dual(
        cyan,
        vec![body("  diff shows exactly where", theme)],
        red,
        vec![styled("    found    ", body_c), styled("`&str`", yellow)],
    ));
    l.push(dual(
        cyan,
        vec![body("  output differs.", theme)],
        red,
        vec![body("  Parsed with file/line info.", theme)],
    ));
    ...
    l.push(top(
        "State Assertions (for environment exercises)",
        FULL,
        green,
    ));
    ...
    l.push(row(
        green,
        vec![styled("  ✓ ", green), body("File output.txt exists", theme)],
        FULL,
    ));
    l.push(row(
        green,
        vec![
            styled("  ✓ ", green),
            body("File contains \"success\"", theme),
        ],
        FULL,
    ));
    l.push(row(
        ...
        vec![
            styled("  ✓ ", green),
            body("Directory backup/ exists", theme),
        ],
        FULL,
    ));
    l.push(row(
        ...
        vec![
            styled("  ✓ ", green),
            body("Symlink latest → backup/2024-01-15", theme),
        ],
        FULL,
    ));
    l.push(row(
        ...
        vec![
            styled("  ✘ ", red),
            body("File permissions are 0644 (got 0755)", theme),
        ],
        FULL,
    ));
    l.push(row(
        ...
        vec![
            styled("  ✘ ", red),
            body("Line count matches expected (got 3, want 5)", theme),
        ],
        FULL,
    ));
    l.push(row(green, vec![], FULL));
    l.push(row(
        green,
        vec![body(
            "  Courses can validate: output text, file existence, contents, permissions,",
            theme,
        )],
        FULL,
    ));
    l.push(row(
        green,
        vec![body(
            "  symlinks, directory state, file counts, regex patterns, and more.",
            theme,
        )],
        FULL,
    ));
    ...
    l.push(Line::from(vec![
        body(
            "   Every error is a learning opportunity — feedback ",
            theme,
        ),
...
fn slide_environment(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   Courses can simulate real development environments — not just compile-and-run:",
        theme,
    )));
    l.push(blank());
    ...
    l.push(Line::from(vec![
        ...
        body("     ", theme),
        bold("What courses can do:", cyan),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", yellow),
        styled(
            &format!("{:<w$}", " Create files and directories", w = pw),
            body_c,
        ),
        styled("│", yellow),
    ]));
    l.push(Line::from(vec![
        ...
        styled("│", yellow),
        styled(
            &format!("{:<w$}", " Set environment variables", w = pw),
            body_c,
        ),
        styled("│", yellow),
        body("     ", theme),
        styled("→ ", green),
        body("Create project scaffolding", theme),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", yellow),
        styled(&format!("{:<w$}", " Run setup commands", w = pw), body_c),
        styled("│", yellow),
        body("     ", theme),
        styled("→ ", green),
        body("Pre-populate config files", theme),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", yellow),
        styled(&format!("{:<w$}", " Create symlinks", w = pw), body_c),
        styled("│", yellow),
        body("     ", theme),
        styled("→ ", green),
        body("Initialize databases", theme),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        bold(
            &format!("└{}┬{}┘", "─".repeat(pw / 2), "─".repeat(pw - pw / 2 - 1)),
            yellow,
        ),
        body("     ", theme),
        styled("→ ", green),
        body("Start background servers", theme),
    ]));
    // Connector
    l.push(Line::from(vec![
        ...
        body("                       ", theme),
        styled("→ ", green),
        body("Allocate dynamic ports", theme),
    ]));
    ...
    l.push(Line::from(vec![
        ...
        body("     ", theme),
        styled("→ ", green),
        body("Validate filesystem state", theme),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", green),
        styled(
            &format!("{:<w$}", " Start background services", w = pw),
            body_c,
        ),
        styled("│", green),
        body("     ", theme),
        styled("→ ", green),
        body("Check file permissions", theme),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", green),
        styled(
            &format!("{:<w$}", " Allocate dynamic ports", w = pw),
            body_c,
        ),
        styled("│", green),
    ]));
    l.push(Line::from(vec![
        ...
        styled("│", green),
        bold(
            &format!("{:<w$}", " ══► YOUR CODE RUNS HERE ══►", w = pw),
            Color::White,
        ),
        styled("│", green),
    ]));
    l.push(Line::from(vec![
        ...
        styled("│", green),
        styled(
            &format!("{:<w$}", " Sandboxed with loopback network", w = pw),
            body_c,
        ),
        styled("│", green),
        body("     ", theme),
        bold("Sandboxing tiers:", cyan),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        bold(
        ...
        body("     ", theme),
        styled("  Basic:     ", yellow),
        body("tmpdir + timeout", theme),
    ]));
    l.push(Line::from(vec![
        body(&format!("       {}│", " ".repeat(pw / 2 + 1)), theme),
        body("                       ", theme),
        styled("  Contained: ", yellow),
        body("firejail / bubblewrap", theme),
    ]));
    ...
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", muted_c),
        styled(&format!("{:<w$}", " Teardown + cleanup", w = pw), body_c),
        styled("│", muted_c),
    ]));
    l.push(Line::from(vec![
        Span::raw("       ".to_string()),
        styled("│", muted_c),
        styled(
            &format!("{:<w$}", " Validate final state assertions", w = pw),
            body_c,
        ),
        styled("│", muted_c),
    ]));
    ...
    l.push(Line::from(vec![
        ...
        body(
            " — courses run in isolated temp directories, can never affect your system.",
            theme,
        ),
    ]));
...
fn slide_ai_tutor(theme: &Theme) -> Vec<Line<'static>> {
    ...
    #[cfg(feature = "llm")]
    {
        ...
        l.push(blank());
        l.push(Line::from(body(
            "   Stuck? Press [a] to chat with an AI that understands your full context:",
            theme,
        )));
        l.push(blank());
        ...
        l.push(row(
            ...
            vec![
                styled("  You: ", yellow),
                styled(
                    "My function returns 0 instead of the sum. What am I doing wrong?",
                    body_c,
                ),
            ],
            FULL,
        ...
        l.push(row(
            ...
            vec![
                styled("  AI: ", green),
                styled(
                    "Looking at your code, I see the issue — you declared `sum` inside",
                    body_c,
                ),
            ],
            FULL,
        ...
        l.push(row(
            cyan,
            vec![styled(
                "      the loop, so it resets to 0 each iteration. Try moving the",
                body_c,
            )],
            FULL,
        ));
        l.push(row(
            cyan,
            vec![styled(
                "      declaration before the for-loop. Think about variable scope —",
                body_c,
            )],
            FULL,
        ));
        l.push(row(
            cyan,
            vec![styled(
                "      where does `sum` need to live to accumulate across iterations?",
                body_c,
            )],
            FULL,
        ));
        ...
        l.push(dual(
            cyan,
            vec![styled("  → ", green), body("Lesson content", theme)],
            green,
            vec![
                styled("  → ", green),
                body("Runs 100% on your machine", theme),
            ],
        ));
        l.push(dual(
            cyan,
            vec![styled("  → ", green), body("Your current code", theme)],
            green,
            vec![styled("  → ", green), body("No data sent anywhere", theme)],
        ));
        l.push(dual(
            cyan,
            vec![
                styled("  → ", green),
                body("Compiler errors & output", theme),
            ],
            green,
            vec![
                styled("  → ", green),
                body("Guides without spoiling", theme),
            ],
        ));
        l.push(dual(
            cyan,
            vec![styled("  → ", green), body("Exercise requirements", theme)],
            green,
            vec![styled("  → ", green), body("Works in sandboxes too", theme)],
        ));
        l.push(dual(
            cyan,
            vec![styled("  → ", green), body("Your attempt history", theme)],
            green,
            vec![styled("  → ", green), body("Enable in Settings [s]", theme)],
        ));
        l.push(dual(cyan, vec![], green, vec![]));
        ...
        l.push(Line::from(vec![
            ...
            styled("Enter", cyan),
            body(" = newline, ", theme),
            styled("Ctrl+Enter", cyan),
            body(" or ", theme),
            styled("Tab", cyan),
            body(" = send. ", theme),
            styled("[Esc]", cyan),
            body(" closes chat.", theme),
        ]));
    }
    ...
    #[cfg(not(feature = "llm"))]
    {
        ...
        l.push(blank());
        l.push(Line::from(body(
            "   LearnLocal supports an optional AI tutor you can enable in Settings.",
            theme,
        )));
        l.push(Line::from(body(
            "   It guides you without spoiling the answer.",
            theme,
        )));
        l.push(blank());
        ...
        l.push(row(
            ...
            vec![
                styled("  → ", green),
                body("Answer questions about the lesson you're reading", theme),
            ],
            FULL,
        ));
        l.push(row(
            ...
            vec![
                styled("  → ", green),
                body(
                    "Help debug your code using your actual errors and output",
                    theme,
                ),
            ],
            FULL,
        ...
        l.push(row(
            ...
            vec![
                styled("  → ", green),
                body(
                    "Guide you toward the solution without giving it away",
                    theme,
                ),
            ],
            FULL,
        ...
        l.push(row(
            ...
            vec![
                styled("  → ", green),
                body("Sees your lesson, code, compiler output, and hints", theme),
            ],
            FULL,
        ));
        l.push(row(muted_c, vec![], FULL));
        l.push(row(
            muted_c,
            vec![body(
                "  Runs 100% on your machine — no data sent anywhere.",
                theme,
            )],
            FULL,
        ));
        l.push(row(
            muted_c,
            vec![
                body("  Enable via ", theme),
                styled("Settings [s]", green),
                body(" on the home screen.", theme),
            ],
            FULL,
        ));
        l.push(row(muted_c, vec![], FULL));
...
fn slide_progress_combined(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(row(
        ...
        vec![
            body("  Saved at ", theme),
            styled("~/.local/share/learnlocal/progress.json", yellow),
    ...
    l.push(dual(
        ...
        vec![
            styled("  ★ ", yellow),
            body("Exercise → success flash", theme),
        ],
    ));
    l.push(dual(
        green,
        ...
        vec![
            styled("  ★ ", yellow),
            body("Lesson → stats summary", theme),
        ],
    ));
    l.push(dual(
        green,
        ...
        vec![
            styled("  ★ ", yellow),
            body("Course → full celebration", theme),
        ],
    ));
    l.push(dual(green, vec![], yellow, vec![]));
    l.push(dual(
        green,
        vec![bold("  Lesson Sandboxes:", green)],
        yellow,
        vec![],
    ));
    l.push(dual(
        ...
        vec![
            styled("  → ", green),
            body("One playground per lesson", theme),
        ],
        yellow,
        vec![body("  View aggregate stats:", theme)],
    ));
    l.push(dual(
        green,
        vec![
        ...
        yellow,
        vec![styled("  [t]", cyan), body(" Stats from home", theme)],
    ));
    l.push(dual(
        green,
        vec![styled("  → ", green), body("[s] from lesson recap", theme)],
        yellow,
    ...
    l.push(dual(
        ...
        vec![],
        yellow,
        vec![styled("  [r]", cyan), body(" Reset course progress", theme)],
    ));
    l.push(dual(green, vec![], yellow, vec![]));
...
fn slide_get_started(theme: &Theme, courses: &[String]) -> Vec<Line<'static>> {
    ...
    if courses.is_empty() {
        l.push(Line::from(body(
            "   No courses currently installed.",
            theme,
        )));
        l.push(blank());
    } else {
    ...
    l.push(Line::from(body(
        "   Some courses require specific tools or OS — the home screen shows what's ready.",
        theme,
    )));
    l.push(blank());
    ...
    l.push(dual(
        cyan,
        vec![styled("  [h]", cyan), body("      How To reference", theme)],
        green,
        vec![
            styled("  [a]", cyan),
    ...
    l.push(blank());
    l.push(Line::from(muted(&format!("   {}", "─".repeat(75)), theme)));
    l.push(Line::from(muted(
        "   LearnLocal — learn programming offline, in your terminal.",
        theme,
    )));
