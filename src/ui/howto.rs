...
use crate::ui::tour::{
    blank, styled, bold, muted, body, heading, separator,
    row, top, bot, drow, dtop, dbot,
    dual, dual_top, dual_bot,
    FULL,
};
...
fn slide_overview(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(row(yellow, vec![
        body("  ", theme),
        styled("[Space]", cyan), body(" READ  ", theme),
        muted("→  ", theme),
        styled("[e]", cyan), body(" EDIT      ", theme),
        muted("→  ", theme),
        styled("[Enter]", cyan), body(" RUN  ", theme),
        muted("→  ", theme),
        styled("[t]", cyan), body(" SUBMIT", theme),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(row(yellow, vec![
    ...
    l.push(row(yellow, vec![
        body("  Stuck?  ", theme),
        styled("[h]", yellow), body(" hints  →  ", theme),
    ...
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[2]", cyan), body(" Editing Code      ", theme),
        muted("— the built-in inline editor", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[3]", cyan), body(" Running & Testing ", theme),
        muted("— run vs submit, feedback types", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[4]", cyan), body(" Reading Lessons   ", theme),
        muted("— progressive reveal, section focus", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[5]", cyan), body(" Getting Help      ", theme),
        muted("— hints, skipping, optional AI", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[6]", cyan), body(" Files & Settings  ", theme),
        muted("— paths, configuration, sandboxes", theme),
    ]));
    l.push(Line::from(vec![
        body("   ", theme),
        styled("[7]", cyan), body(" Key Reference     ", theme),
...
fn slide_editing(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(Line::from(body(
        "   Press [e] to open the built-in inline editor — edit right inside the TUI:", theme,
    )));
    l.push(blank());
    ...
    l.push(row(green, vec![
        styled("  5 │ ", muted_c), styled("    cout << \"Hello!\" << endl;", blue),
    ], FULL));
    l.push(row(green, vec![
    ...
    l.push(dual(
        cyan, vec![styled("  Del   ", green), body("  Delete forward", theme)],
    ...
    l.push(dual(
        ...
        yellow, vec![styled("  →", green), body(" Esc saves and runs —", theme)],
    ));
    l.push(dual(
...
fn slide_running(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(dual(
        cyan, vec![body("  Executes your code and", theme)],
        green, vec![body("  Validates your solution", theme)],
    ));
    l.push(dual(
        cyan, vec![body("  shows the output.", theme)],
        green, vec![body("  against expected output.", theme)],
    ));
    l.push(dual(cyan, vec![], green, vec![]));
    l.push(dual(
        cyan, vec![styled("  No grading", yellow), body(" — experiment", theme)],
        green, vec![styled("  ✓ Pass", green), body(" → next exercise", theme)],
    ));
    l.push(dual(
        cyan, vec![body("  freely, as often as you", theme)],
        green, vec![styled("  ✘ Fail", red), body(" → detailed feedback", theme)],
    ));
    l.push(dual(
        cyan, vec![body("  want.", theme)],
        green, vec![],
    ));
    l.push(dual(cyan, vec![], green, vec![]));
    l.push(dual(
        cyan, vec![body("  Use this to debug and", theme)],
        green, vec![body("  Only submit when you're", theme)],
    ));
    l.push(dual(
        cyan, vec![body("  develop your solution.", theme)],
        green, vec![body("  confident it's correct.", theme)],
    ));
    l.push(dual(cyan, vec![], green, vec![]));
    ...
    l.push(row(yellow, vec![], FULL));
    l.push(row(yellow, vec![
        bold("  Output Diff", cyan),
    ], FULL));
    l.push(row(yellow, vec![
        body("  Color-coded comparison:  ", theme),
        styled("Expected: ", muted_c), styled("Hello, World!", green),
    ], FULL));
    l.push(row(yellow, vec![
        body("                           ", theme),
        styled("Got:      ", muted_c), styled("hello world", red),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(row(yellow, vec![
        bold("  Compiler Diagnostics", cyan),
    ], FULL));
    l.push(row(yellow, vec![
        body("  Parsed errors with file/line info:  ", theme),
        styled("error", red), body(" → main.rs:3:12", theme),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(row(yellow, vec![
    ...
    l.push(row(yellow, vec![
        body("  ", theme),
        styled("✓", green), body(" File exists  ", theme),
        styled("✓", green), body(" Content matches  ", theme),
        styled("✓", green), body(" Permissions correct  ", theme),
        styled("✘", red), body(" Missing", theme),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(bot(FULL, yellow));
    l.push(blank());
    ...
    l.push(row(Color::Blue, vec![
        body("  Command exercises open a terminal instead of the code editor.", theme),
    ], FULL));
    l.push(row(Color::Blue, vec![
    ...
    l.push(row(Color::Blue, vec![], FULL));
    l.push(row(Color::Blue, vec![
        styled("  [Enter]", cyan), body("   Execute command       ", theme),
        styled("[↑/↓]", cyan), body("     Command history", theme),
    ], FULL));
    l.push(row(Color::Blue, vec![
        styled("  [Ctrl+H]", cyan), body("  Reveal hint            ", theme),
        styled("[Ctrl+C]", cyan), body("   Clear input", theme),
    ], FULL));
    l.push(row(Color::Blue, vec![
        styled("  [F1]", cyan), body("      Help overlay           ", theme),
        styled("[Esc]", cyan), body("      Exit shell mode", theme),
    ], FULL));
    l.push(row(Color::Blue, vec![], FULL));
    ...
    l.push(Line::from(vec![
        ...
        styled("why", green),
        body(" — not just \"wrong\". Use feedback to learn and iterate.", theme),
    ]));
...
fn slide_reading(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   Lesson content is revealed one section at a time — absorb, then advance:", theme,
    )));
    l.push(blank());
    ...
    l.push(row(cyan, vec![
        body("    ", theme),
        bold("▐", green),
        styled(" Variables store data. In C++, you declare them with a type...", green),
    ], FULL));
    l.push(row(cyan, vec![
        body("    ", theme),
        styled("▐", muted_c),
    ...
    l.push(row(cyan, vec![
        body("    ", theme),
        styled("▐", muted_c),
        styled(" C++ has several built-in types: int, double, char, bool...", muted_c),
    ], FULL));
    l.push(row(cyan, vec![
        body("    ", theme),
        styled("▐", muted_c),
    ...
    l.push(row(cyan, vec![
        body("    ", theme),
        styled("░  Section 3: hidden — press [Space] to reveal   ░", muted_c),
    ], FULL));
    l.push(row(cyan, vec![
        body("    ", theme),
        styled("░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░", muted_c),
    ...
    l.push(dual(
        ...
        yellow, vec![styled("  →", green), body(" Focused section is bright", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [↑/↓]  ", cyan), body("  Focus between sections", theme)],
        yellow, vec![styled("  →", green), body(" Earlier sections are dimmed", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [PgUp] ", cyan), body("  Scroll up a page", theme)],
        yellow, vec![styled("  →", green), body(" Hidden sections show as", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [PgDn] ", cyan), body("  Scroll down a page", theme)],
        yellow, vec![body("    blocked until revealed", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [Home] ", cyan), body("  Jump to top", theme)],
        yellow, vec![],
    ));
    l.push(dual(
        green, vec![styled("  [End]  ", cyan), body("  Jump to bottom", theme)],
        yellow, vec![styled("  →", green), body(" Read at your own pace!", theme)],
    ));
    l.push(dual(green, vec![], yellow, vec![]));
...
fn slide_help(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(blank());
    l.push(Line::from(body(
        "   When you're stuck, LearnLocal offers a graduated support path:", theme,
    )));
    l.push(blank());
    ...
    l.push(Line::from(vec![
        Span::raw("            ".to_string()),
        bold("│", yellow), styled(&format!("{:<w$}", " [h] HINTS", w = pw), yellow), bold("│", yellow),
        body("  ────────→  ", theme),
        bold("│", green), styled(&format!("{:<w$}", " [s] SKIP", w = pw), green), bold("│", green),
    ]));
    l.push(Line::from(vec![
        Span::raw("            ".to_string()),
        bold("│", yellow), body(&format!("{:<w$}", " Reveals one hint at a time", w = pw), theme), bold("│", yellow),
        Span::raw("             ".to_string()),
        bold("│", green), body(&format!("{:<w$}", " Come back to it later", w = pw), theme), bold("│", green),
    ]));
    l.push(Line::from(vec![
        Span::raw("            ".to_string()),
        bold(&format!("└{}┘", "─".repeat(pw)), yellow),
    ...
    l.push(row(yellow, vec![
        ...
        styled("[h]", cyan),
        body(" reveals one more hint. Try to solve with fewer hints.", theme),
    ], FULL));
    l.push(row(yellow, vec![
        body("  Hints range from gentle nudges to near-solutions.", theme),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(bot(FULL, yellow));
    ...
    l.push(row(cyan, vec![], FULL));
    l.push(row(cyan, vec![
        body("  Chat with an AI that sees your lesson, code, errors, and hints.", theme),
    ], FULL));
    l.push(row(cyan, vec![
        body("  Guides you toward the answer without spoiling it.", theme),
    ], FULL));
    l.push(row(cyan, vec![
        body("  Runs on your machine — no data sent anywhere. Enable in ", theme),
    ...
    l.push(row(green, vec![], FULL));
    l.push(row(green, vec![
        styled("  [?]", cyan), body("  Quick keyboard shortcut overlay (inside a course)", theme),
    ], FULL));
    l.push(row(green, vec![
        styled("  [r]", cyan), body("  Reset exercise to starter code if you want a fresh start", theme),
    ], FULL));
    l.push(row(green, vec![], FULL));
    l.push(bot(FULL, green));
...
fn slide_files(theme: &Theme, ctx: &HowToCtx) -> Vec<Line<'static>> {
    ...
    l.push(row(cyan, vec![], FULL));
    l.push(row(cyan, vec![
        styled("  Config      ", yellow), body(&ctx.config_path, theme),
    ], FULL));
    l.push(row(cyan, vec![
        styled("  Progress    ", yellow), body(&ctx.progress_path, theme),
    ], FULL));
    l.push(row(cyan, vec![
        styled("  Sandboxes   ", yellow), body(&ctx.sandbox_path, theme),
    ], FULL));
    l.push(row(cyan, vec![
        styled("  Courses     ", yellow), body(&ctx.courses_path, theme),
    ], FULL));
    l.push(row(cyan, vec![], FULL));
    l.push(bot(FULL, cyan));
    l.push(blank());
    ...
    l.push(dual_top("Progress Tracking", green, "Drafts & Sandboxes", yellow));
    l.push(dual(green, vec![], yellow, vec![]));
    l.push(dual(
        green, vec![styled("  →", green), body(" Saved automatically", theme)],
        yellow, vec![styled("  →", green), body(" Draft code saved on exit", theme)],
    ));
    l.push(dual(
        green, vec![styled("  →", green), body(" Pick up where you left", theme)],
        yellow, vec![styled("  →", green), body(" Restored when you return", theme)],
    ));
    l.push(dual(
        green, vec![body("    off, any time", theme)],
        yellow, vec![styled("  →", green), body(" Cleared on completion", theme)],
    ));
    l.push(dual(
        green, vec![styled("  →", green), body(" Keyed to course + major", theme)],
        yellow, vec![],
    ));
    l.push(dual(
        green, vec![body("    version (survives patches)", theme)],
        yellow, vec![bold("  Lesson Sandboxes:", yellow)],
    ));
    l.push(dual(
        green, vec![],
        yellow, vec![styled("  →", green), body(" Free playground per lesson", theme)],
    ));
    l.push(dual(
        green, vec![body("  View progress:", theme)],
        yellow, vec![styled("  →", green), body(" No grading, just explore", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [p]", cyan), body(" Progress details", theme)],
        yellow, vec![styled("  →", green), body(" Code persists between", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [t]", cyan), body(" Stats overview", theme)],
        yellow, vec![body("    sessions", theme)],
    ));
    l.push(dual(
        green, vec![styled("  [r]", cyan), body(" Reset (on Progress)", theme)],
        yellow, vec![],
    ));
    l.push(dual(green, vec![], yellow, vec![]));
    l.push(dual_bot(green, yellow));
...
fn slide_keys(theme: &Theme) -> Vec<Line<'static>> {
    ...
    l.push(dual(
        cyan, vec![styled("  [h]    ", cyan), body("  How To (this!)", theme)],
    ...
    l.push(row(yellow, vec![], FULL));
    l.push(row(yellow, vec![
        styled("  Arrows", cyan), body("  Move cursor          ", theme),
        styled("Esc", cyan), body("       Save & close", theme),
    ], FULL));
    l.push(row(yellow, vec![
        styled("  Type  ", cyan), body("  Insert text          ", theme),
        styled("Ctrl+S", cyan), body("    Save (stay open)", theme),
    ], FULL));
    l.push(row(yellow, vec![
        styled("  Bksp  ", cyan), body("  Delete backward      ", theme),
        styled("Home/End", cyan), body("  Line start/end", theme),
    ], FULL));
    l.push(row(yellow, vec![], FULL));
    l.push(bot(FULL, yellow));
    l.push(blank());
    ...
    l.push(row(Color::Blue, vec![], FULL));
    l.push(row(Color::Blue, vec![
        styled("  Enter ", cyan), body("  Run command          ", theme),
        styled("↑/↓", cyan), body("       Command history", theme),
    ], FULL));
    l.push(row(Color::Blue, vec![
        styled("  Ctrl+H", cyan), body(" Reveal hint           ", theme),
